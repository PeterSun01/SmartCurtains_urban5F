...
static mdf_err_t mqtt_update_subdev(uint8_t type, uint8_t *subdev_list, size_t subdev_num,ListNode *h)
{
...
mdf_err_t mesh_mqtt_subscribe(uint8_t *subdev_list, size_t subdev_num,ListNode *h)
{
    return mqtt_update_subdev(SUBSCRIBED, subdev_list, subdev_num,h);
}

mdf_err_t mesh_mqtt_unsubscribe(uint8_t *subdev_list, size_t subdev_num,ListNode *h)
{
    return mqtt_update_subdev(UNSUBSCRIBED, subdev_list, subdev_num,h);
}
...
void insert(ListNode *h,char *in_mac_info,char *in_id_info,char *in_channel_info)
{
	 ...
	 while(p!=NULL)
     { 
	   if(!strncmp(p->mac_info,in_mac_info,12))
       {
           //printf("already hava return \r\n");
            return; 
       }
     ...
     memset(q->mac_info,'\0', sizeof(q->mac_info));
	 strncpy(q->mac_info,in_mac_info,12);

     memset(q->id_info,'\0', sizeof(q->id_info));
	 strncpy(q->id_info,in_id_info,12);
	 
     memset(q->channel_info,'\0', sizeof(q->channel_info));
     strncpy(q->channel_info,in_channel_info,12);
...
void del(ListNode *h, char *mac_info)
{
    ListNode *p, *q;
    p = h->next;
    q = h;
    while (p != NULL)
    {
        if (!strncmp(p->mac_info, mac_info, 12))
            break;
        p = p->next;
        q = q->next;
    }
...
ListNode *find_mac(ListNode *h, char *mac_info)
{
    ListNode *p, *q;
    p = h->next;
...
ListNode *find_id_channel(ListNode *h, char *mac_info)
{
    ListNode *p, *q;
    p = h->next;
...
void show(ListNode *h)
{
	 ...
	 if(p==NULL)
	 {
		 printf("no more student info\n");
		 return;
	 }
	 
	 while(p)
     {
	   printf("%-12s  %-12s   %-12s   \r\n",p->mac_info,p->id_info,p->channel_info);
	   p=p->next;
	  }
}